<script lang="ts">
	// Error state for the dashboard: message + retry (DESIGN.md > Empty / Loading / Error states).
...
>
	...
	<h2 class="text-xl font-semibold">Could not load the dashboard</h2>
	<p class="text-sm text-muted-foreground">{message}</p>
	<Button variant="outline" size="sm" class="max-sm:h-11" onclick={() => invalidateAll()}>
		Try again
	</Button>
</div>
